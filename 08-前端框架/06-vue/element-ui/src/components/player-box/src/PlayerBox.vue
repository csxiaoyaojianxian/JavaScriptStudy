<template>
    <div class="pop" id="playerBoxItem">
        <div class="pop-panel">
...
export default {
    name: 'PlayerBox',
    props: ["pid", "tid", "currentPlayerIdList"],
    data() {
        return {
            playerList: [],
            selectedIdList: this.currentPlayerIdList
        };
    },
    watch: {
        'currentPlayerIdList': function () {
            this.selectedIdList = this.currentPlayerIdList
        }
    },
    created: function () {
    ...
    methods: {
        ...
        updatePlayer: function () {
            this.$http.jsonp('https://apps.game.qq.com/cf/a20161220ms/pmatch.php?action=update_player', {
                params: {
                    pid: this.pid,
                    teamId: this.tid,
        ...
        loadAllPlayerList: function () {
            this.$http.jsonp('https://apps.game.qq.com/cf/a20161220ms/player.php?action=get_all', {
                params: {
                    offset: this.offset,
                    count: 10
                }
